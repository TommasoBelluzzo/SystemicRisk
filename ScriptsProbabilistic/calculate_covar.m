...
function [covar,dcovar] = calculate_covar(r0_m,r0_x,s_x,k,svars)

    t = length(r0_m);

    var_x = s_x * quantile((r0_x ./ s_x),k);

    if (nargin < 5)
        [beta,~,~,~] = quantile_regression(r0_m,r0_x,k);
        covar = (beta(1) + (beta(2) .* var_x)) .* -1;
    else
        [beta,~,~,~] = quantile_regression(r0_m,[r0_x svars],k);
    ...
    dcovar = (beta(2) .* (var_x - repmat(median(r0_x),t,1))) .* -1;
