% [INPUT]
% y = A float t-by-n matrix containing the time series to be sanitized.
% x = A vector of length t containing the observation points of the time series (if empty, observations are assumed to be linearly spaced between 1 and t).
...
%
% [OUTPUT]
% y = A float t-by-n matrix containing the sanitized time series.

function y = sanitize_data(varargin)
    ...
    ipr = ip.Results;
    y = ipr.y;
    [x,w,m] = validate_input(y,ipr.x,ipr.w,ipr.m);

    nargoutchk(1,1);

    y = sanitize_data_internal(y,x,w,m);

end

function y = sanitize_data_internal(y,x,w,m)

    for i = 1:size(y,2)
        ys = y(:,i);

        nan_indices = isnan(ys);
        ...
        if (any(nan_indices))
            ys = fill_missing_values(ys,x,nan_indices);
        end
        
        if (~isempty(w))
            ys = replace_outliers(ys,x,w);
        end
        
        if (~isempty(m))
            ys = min(max(ys,m(1)),m(2));
        end
...
function [x,w,m] = validate_input(y,x,w,m)
