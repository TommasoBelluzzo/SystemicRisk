...
function main_network(varargin)
    ...
    main_network_internal(ipd.data,tpl,res,ipd.sst,ipd.rob,ipd.anl);

end

function main_network_internal(data,tpl,res,sst,rob,anl)
...
function plot_indicators(data)
    ...
    hold off;
    legend(sub_2,[ar_1 ar_2],'Num IO','Num IOO','Location','best');
    ...
    idx = ~isnan(data.NumIO);

    if (length(unique(year(data.DatesNum(idx)))) <= 3)
